...
const find_room_id = async (req, res) => {
    ...
    // console.log(arr, nickname, opponent);
    return [arr, nickname, opponent];
}
...
const get_room_id = async (req, res) => {
    ...
    copy_room_id = null;
    let [arr, nickname, opponent] = await find_room_id(req, res);
...
let send_chat = async (req, res) => {
    ...
    const room_id = req.session.chat.room_id;
    const id = req.session.USER[0];
    if(!check_id(req, res)) return res.redirect('/error');
...
let update_room_id = req => {
    copy_room_id = req.session.chat.room_id;
    my_id = req.session.USER[0];
}
...
let make_table = async (req, res) => {
    ...
    req.session.chat = {};
    const userconnection = await user_pool.getConnection(async(conn) => conn);
    await userconnection.query(`SELECT SELLER_ID FROM PRODUCT WHERE PRODUCT_ID = '${req.body.id}'`)
    .then(row => req.session.chat.away_id = row[0][0]["SELLER_ID"])
    .catch(error => {
        console.log(error)
        return res.redirect("/error");
    });
    for (let i = 0; i < opponent.length; i++) {
        // console.log(i, req.session.chat.away_id);
        if(req.session.chat.away_id == opponent[i]) {
            req.session.chat.room_id = arr[i];
            req.session.chat.away_nickname = nickname[i];
            userconnection.release();
            return res.json({url: '/chat/room'});
        }
    }
    await userconnection.query(`SELECT NICKNAME, iv FROM USER WHERE USER_ID = '${req.session.chat.away_id}'`)
    .then(async row => req.session.chat.away_nickname = await crypto.decrypt_aes(row[0][0]["NICKNAME"], await crypto.convert_buffer(row[0][0]["iv"])))
    .catch(error => {
        console.log(error)
        return res.redirect("/error");
    });
...
module.exports = {get_room_id, make_room, check_room, send_chat, update_room_id, ret_room_id, make_table, ret_my_id};